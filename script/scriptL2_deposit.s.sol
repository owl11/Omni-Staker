...
contract StakerL2_deposit is Script {
    ...
    function run() public {
        ...
        uint256 privateKey;
        MessagingFee memory fee = staker.estimateFee(_tokensToSend, l2oApp);
        privateKey = vm.envUint("PRIVATE_KEY");
        vm.startBroadcast(privateKey);
        IERC20(l2oApp).approve(address(l2staker), _tokensToSend);
        staker.updateThreshold(_tokensToSend);

        staker.deposit(_tokensToSend);
        vm.stopBroadcast();
        ...
        IERC20(l2oApp).approve(address(l2staker), _tokensToSend);
        staker.deposit(_tokensToSend);
        staker.transferWithoutQuote{value: fee.nativeFee}(fee);
